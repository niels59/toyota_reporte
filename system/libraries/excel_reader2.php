...
class Spreadsheet_Excel_Reader {
	...
	// =========================================
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='dataTable',$id='table_excel') {
		$out = "<table class=\"$table_class\" cellspacing=0 id=\"$id\" >";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
			if ($row_numbers) {
				$out .= "\n\t\t<th class='heading'>&nbsp</th>";
			}
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$out .= "<nobr id='".nl2br($val)."_id'>".nl2br($val)."</nobr>";
					$out .= "</td>";
				}
			}
			$out .= "</tr>\n";
